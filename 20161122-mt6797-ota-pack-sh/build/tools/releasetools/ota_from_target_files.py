...
class Item(object):
 ...
 #add by tzh
def CopySuperSensorCfg(input_zip, output_zip=None,substitute=None):
  
  symlinks = []

  for info in input_zip.infolist():
    if info.filename.startswith("SUPERSENSOR-CFG/"):
      basefilename = info.filename[16:]
      if IsSymlink(info):
        symlinks.append((input_zip.read(info.filename),
                         "/SUPERSENSOR-CFG/" + basefilename))
      else:
	import copy
        info2 = copy.copy(info)
        fn = info2.filename = "SUPERSENSOR-CFG/" + basefilename
...
#tzh add for  full ota supersensor
  script.Print("UnpackPackageDir  supersensor cfg  files...")
  script.Mount("/system")
  script.UnpackPackageDir("SUPERSENSOR-CFG", "/system/supersensor")
  script.AppendExtra("set_metadata_recursive(\"/system/supersensor\", \"uid\", 1000, \"gid\", 1000, \"mode\", 0770, \"capabilities\", 0x0);")
  CopySuperSensorCfg(input_zip, output_zip)
...
#tzh add for in  ota  supersensor
  script.Print("UnpackPackageDir  supersensor cfg  files...")
  script.Mount("/system")
  script.UnpackPackageDir("SUPERSENSOR-CFG", "/system/supersensor")
  CopySuperSensorCfg(source_zip, output_zip)
  script.AppendExtra("set_metadata_recursive(\"/system/supersensor\", \"uid\", 1000, \"gid\", 1000, \"mode\", 0770, \"capabilities\", 0x0);")
  script.Unmount("/system")
